...
aframeReady(() => {
  AFRAME.registerComponent("matrix-map", {
    ...
    buildBase() {
      ...
      base.setAttribute("geometry",`primitive: cylinder; radius: ${this.data.radius}; height: 0.6`);
      base.setAttribute("position","0 -0.35 0");
      base.setAttribute('grid-material','cellSize:2; lineThickness:0.12; roughness:0.94; metalness:0.03');
      this.el.appendChild(base);
    },
    buildLinear(){
      // Safe zone floor (no spawns) - rectangular pad
      const safe = document.createElement('a-entity');
      safe.setAttribute('geometry',`primitive: box; width:${this.data.corridorWidth}; height:0.4; depth:${this.data.safeLength}`);
      safe.setAttribute('position',`0 0 ${this.data.safeLength/2}`);
      safe.setAttribute('grid-material','cellSize:2; lineThickness:0.12; roughness:0.94; metalness:0.03');
      this.el.appendChild(safe);

      // Corridor segments beyond safe zone (walkway)
      const segmentLen = 18;
      for(let z = this.data.safeLength; z < this.data.forwardLength; z += segmentLen){
        const seg = document.createElement('a-entity');
        const depth = Math.min(segmentLen, this.data.forwardLength - z);
        seg.setAttribute('geometry',`primitive: box; width:${this.data.corridorWidth}; height:0.32; depth:${depth}`);
        seg.setAttribute('position',`0 0 ${z + depth/2}`);
        seg.setAttribute('grid-material','cellSize:2; lineThickness:0.12; roughness:0.93; metalness:0.03');
        this.el.appendChild(seg);
      }

      // Side walls (simple low barriers) for guidance
      const wallInterval = 24;
      for(let z = this.data.safeLength + 4; z < this.data.forwardLength; z += wallInterval){
        const left = document.createElement('a-entity');
        left.setAttribute('geometry',`primitive: box; width:0.6; height:2; depth:14`);
        left.setAttribute('position',`${-this.data.corridorWidth/2 + 0.4} 1 ${z}`);
        left.setAttribute('material','color:#2e2e2e; roughness:0.85; metalness:0.05');
        this.el.appendChild(left);
        const right = document.createElement('a-entity');
        right.setAttribute('geometry',`primitive: box; width:0.6; height:2; depth:14`);
        right.setAttribute('position',`${this.data.corridorWidth/2 - 0.4} 1 ${z+7}`);
        right.setAttribute('material','color:#2e2e2e; roughness:0.85; metalness:0.05');
      ...
      for(let b=0; b<bands; b++){
        const zStart = this.data.spawnStartZ + b * ((this.data.forwardLength - this.data.spawnStartZ)/bands);
        const zEnd = this.data.spawnStartZ + (b+1) * ((this.data.forwardLength - this.data.spawnStartZ)/bands);
        const localPlatforms = Math.max(2, Math.floor(this.data.platformCount / bands));
        for(let i=0;i<localPlatforms;i++){
          const plat = document.createElement('a-entity');
          const w = this.data.minSize + this.rand()*(this.data.maxSize - this.data.minSize);
          const d = this.data.minSize + this.rand()*(this.data.maxSize - this.data.minSize);
          const zPos = zStart + this.rand()*(zEnd - zStart);
            const lateral = (this.rand()*2 -1) * (this.data.corridorWidth*0.45);
          plat.setAttribute('geometry',`primitive: box; width:${w}; height:0.45; depth:${d}`);
          const h = this.data.minHeight + this.rand()*(this.data.maxHeight - this.data.minHeight);
          plat.setAttribute('position',`${lateral} ${h} ${zPos}`);
          plat.setAttribute('rotation',`0 ${this.rand()*360} 0`);
          plat.setAttribute('grid-material','cellSize:2; lineThickness:0.12; roughness:0.9; metalness:0.05');
  ...
  plat.setAttribute('grid-material','cellSize:2; lineThickness:0.12; roughness:0.9; metalness:0.05');
    ...
    buildSquare(){
      ...
      floor.setAttribute('geometry',`primitive: box; width:${side}; height:0.4; depth:${side}`);
      floor.setAttribute('position',`0 0 0`);
      floor.setAttribute('grid-material','cellSize:2; lineThickness:0.12; roughness:0.94; metalness:0.03');
      ...
      entrance.setAttribute('geometry',`primitive: box; width:${Math.min(side*0.4, half*0.8)}; height:0.42; depth:${entranceDepth}`);
      entrance.setAttribute('position',`0 0 ${-half - entranceDepth/2 + 0.2}`);
      entrance.setAttribute('grid-material','cellSize:2; lineThickness:0.12; roughness:0.95; metalness:0.03');
      this.el.appendChild(entrance);

      // Perimeter low walls (optional) - simple corners & edges (skip entrance gap)
      const wallH=2, wallT=0.6;
      const edges=[
      ...
      edges.forEach(e=>{
        ...
        wEnt.setAttribute('geometry',`primitive: box; width:${e.w}; height:${wallH}; depth:${e.d}`);
        wEnt.setAttribute('position',`${e.x} ${wallH/2} ${e.z}`);
        wEnt.setAttribute('material','color:#2e2e2e; roughness:0.85; metalness:0.05');
      ...
      for(let i=0;i<plats;i++){
        ...
        const h = this.data.minHeight + this.rand()*(this.data.maxHeight - this.data.minHeight);
        plat.setAttribute('geometry',`primitive: box; width:${w}; height:0.4; depth:${d}`);
    ...
    buildSpawnPointsSquare(half, side){
      ...
      const valid = this.platforms.filter(p=>{ const pos=p.getAttribute('position'); return !(pos.z < -half*0.7 && Math.abs(pos.x) < side*0.25); });
      valid.slice(0, Math.min(24, valid.length)).forEach(p=>{
        const pos=p.getAttribute('position');
        const sp=document.createElement('a-entity');
        sp.setAttribute('position',`${pos.x} ${Number(pos.y)+0.45} ${pos.z}`);
        sp.setAttribute('class', this.data.spawnClass);
        sp.setAttribute('geometry','primitive:ring; radiusInner:0.25; radiusOuter:0.42; segmentsTheta:30');
        sp.setAttribute('material','color:#3eff8c; emissive:#0a391b; opacity:0.4; transparent:true');
        this.el.appendChild(sp);
