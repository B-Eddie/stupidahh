...
(function () {
  ...
  A.registerComponent("tactical-map", {
    ...
    createFloor(size) {
      if (!this.data.showFloor) return;
      // Visible ground slab with grid material aesthetic.
      const floor = document.createElement("a-entity");
      floor.setAttribute(
        "geometry",
        `primitive: box; width:${size}; height:0.25; depth:${size}`
      );
      floor.setAttribute("position", "0 -0.125 0"); // top surface at y=0
      // Use grid-material if available for consistent theme; fallback plain color.
      if (AFRAME.components["grid-material"]) {
        floor.setAttribute(
          "grid-material",
          "cellSize:2; lineThickness:0.12; bgColor:#303234; lineColor:#55595e; roughness:0.9; metalness:0.05"
        );
      } else {
        floor.setAttribute(
          "material",
          "color:#303234; roughness:0.9; metalness:0.05"
        );
      }
    ...
    createPerimeterWalls(size) {
      ...
      const makeSide = (axis, constantVal, alongLength, isHorizontal) => {
        ...
        if (isHorizontal) {
          wallA.setAttribute(
            "geometry",
            `primitive: box; width:${seg}; height:${height}; depth:${thickness}`
          );
          wallA.setAttribute(
            "position",
            `${-half + seg / 2} ${height / 2} ${constantVal}`
          );
          wallB.setAttribute(
            "geometry",
            `primitive: box; width:${seg}; height:${height}; depth:${thickness}`
          ...
          wallA.setAttribute(
            "geometry",
            `primitive: box; width:${thickness}; height:${height}; depth:${seg}`
          );
          wallA.setAttribute(
            "position",
            `${constantVal} ${height / 2} ${-half + seg / 2}`
          );
          wallB.setAttribute(
            "geometry",
            `primitive: box; width:${thickness}; height:${height}; depth:${seg}`
        ...
        [wallA, wallB].forEach((w) => {
          w.setAttribute(
            "material",
            "color:#3a3a3a; roughness:0.8; metalness:0.1; emissive:#1a1a1a; emissiveIntensity:0.1"
          );
    ...
    createCentralStructures() {
      ...
      command.setAttribute(
        "geometry",
        "primitive: box; width:12; height:0.4; depth:12"
      );
      command.setAttribute("position", "0 2 0");
      command.setAttribute(
        "grid-material",
        "cellSize:1.5; lineThickness:0.15; bgColor:#4a4a4a; lineColor:#666666; roughness:0.85; metalness:0.15"
      );
      ...
      pillar.setAttribute(
        "geometry",
        "primitive: cylinder; radius:1.5; height:6"
      );
      pillar.setAttribute("position", "0 3 0");
      pillar.setAttribute(
        "material",
        "color:#2a2a2a; roughness:0.9; metalness:0.05"
      );
    ...
    createRamp(x, y, z, width, height, depth, rotation) {
      ...
      ramp.setAttribute(
        "geometry",
        `primitive: box; width:${width}; height:${depth}; depth:${height}`
      );
      ramp.setAttribute("position", `${x} ${y + height / 2} ${z}`);
      ramp.setAttribute("rotation", `${rotation} 0 0`);
      ramp.setAttribute(
        "grid-material",
        "cellSize:1; lineThickness:0.1; bgColor:#3a3a3a; lineColor:#555555; roughness:0.8; metalness:0.1"
      );
    ...
    createElevatedPlatforms() {
      ...
      platforms.forEach((p, index) => {
        ...
        platform.setAttribute(
          "geometry",
          `primitive: box; width:${p.w}; height:0.4; depth:${p.d}`
        );
        platform.setAttribute("position", `${p.x} ${p.y} ${p.z}`);
        platform.setAttribute("rotation", `0 ${p.rot} 0`);
        platform.setAttribute(
          "grid-material",
          "cellSize:1.5; lineThickness:0.12; bgColor:#4a4a4a; lineColor:#666666; roughness:0.85; metalness:0.15"
        );
    ...
    createCoverElements() {
      ...
      crates.forEach((crate) => {
        for (let i = 0; i < crate.count; i++) {
          ...
          box.setAttribute(
            "geometry",
            "primitive: box; width:1.5; height:1.5; depth:1.5"
          );
          box.setAttribute(
            "position",
            `${crate.x + (Math.random() - 0.5) * 2} ${0.75 + i * 1.5} ${
          ...
          box.setAttribute(
            "material",
            "color:#8B4513; roughness:0.9; metalness:0.05"
          );
      ...
      barriers.forEach((barrier) => {
        ...
        wall.setAttribute(
          "geometry",
          `primitive: box; width:${barrier.w}; height:${barrier.h}; depth:${barrier.d}`
        );
        wall.setAttribute(
          "position",
          `${barrier.x} ${barrier.h / 2} ${barrier.z}`
        );
        wall.setAttribute("rotation", `0 ${barrier.rot} 0`);
        wall.setAttribute(
          "material",
          "color:#555555; roughness:0.8; metalness:0.1"
        );
    ...
    createChokepoints() {
      ...
      passages.forEach((passage) => {
        ...
        wall.setAttribute(
          "geometry",
          `primitive: box; width:${passage.w}; height:${passage.h}; depth:${passage.d}`
        );
        wall.setAttribute(
          "position",
          `${passage.x} ${passage.h / 2} ${passage.z}`
        );
        wall.setAttribute("rotation", `0 ${passage.rot} 0`);
        wall.setAttribute(
          "material",
          "color:#666666; roughness:0.7; metalness:0.2; emissive:#333333; emissiveIntensity:0.1"
        );
        wall.classList.add("solid");
        this.el.appendChild(wall);
      });
    ...
    createSpawnPoints() {
      ...
      spawnPoints.forEach((spawn, index) => {
        ...
        sp.setAttribute(
          "geometry",
          "primitive:ring; radiusInner:0.3; radiusOuter:0.5; segmentsTheta:24"
        );
        sp.setAttribute(
          "material",
          "color:#3eff8c; emissive:#0a391b; opacity:0.6; transparent:true"
        );
  ...
  A.registerComponent("tactical-lighting", {
    ...
    setupLighting() {
      ...
      mainLight.setAttribute(
        "light",
        "type: directional; intensity: 0.8; color: #ffffff; castShadow: true"
      );
      ...
      ambientLight.setAttribute(
        "light",
        "type: hemisphere; intensity: 0.4; color: #87CEEB; groundColor: #2a2a2a"
      );
      ...
      pointLights.forEach((light) => {
        ...
        pointLight.setAttribute(
          "light",
          `type: point; intensity: ${light.intensity}; color: ${light.color}; distance: 25; decay: 2`
        );
