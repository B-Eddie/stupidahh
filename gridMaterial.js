...
(function(){
  ...
  function makeGrid(opts){
    ...
    ctx.strokeStyle=fg; ctx.lineWidth=line; ctx.beginPath();
  ...
  A.registerComponent('grid-material',{
    ...
    apply(){
      ...
      if(!mesh){ this.el.addEventListener('object3dset', e=>{ if(e.detail.type==='mesh') this.apply(); }); return; }
      ...
      const linePx = Math.max(2, Math.round(pxPerCell * this.data.lineThickness * 0.08));
      const url = getTexture({ size: pxPerCell*4, cell: pxPerCell, line: linePx, bg: this.data.bgColor, fg: this.data.lineColor });
      this.el.setAttribute('material',`src:${url}; color:#ffffff; metalness:${this.data.metalness}; roughness:${this.data.roughness}`);
      ...
      if(mesh.material && mesh.material.map){
        ...
        const repZ = Math.max(1, dim.z / this.data.cellSize || dim.y / this.data.cellSize);
