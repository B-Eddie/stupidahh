...
aframeReady(() => {
  AFRAME.registerSystem("goose-timeline", {
    ...
    init() {
      console.warn(
        "[goose-timeline] Temporarily disabled (debugging spawn position issue)"
      );
      ...
      this.sceneEl.addEventListener("geeseSpawned", (e) => {
        ...
        list.forEach((g) => {
          // Assign stable id if missing (store in component's gid property)
          if (!g.hasAttribute("goose-scripted")) {
            const gid = `${Date.now()}_${Math.random().toString(36).slice(2)}`;
            g.setAttribute("goose-scripted", { gid });
          }
          this._geese.push(g);
        });
        if (!this.generated) this.generateTimeline();
      });
    ...
    restartLoop() {
      if (true) return; // disabled during debugging
      this._loopStart = performance.now();
      if (!this.timeline) return;
      // Reset each goose to spawn position
      this.timeline.geese.forEach((entry) => {
        const goose = this._geese.find((g) => {
          const data = g.getAttribute("goose-scripted");
          const gid = data && (typeof data === "string" ? data : data.gid);
          return gid && gid === entry.id;
        });
        if (goose) {
          goose.object3D.position.set(
            entry.spawnPos.x,
            entry.spawnPos.y,
            entry.spawnPos.z
          );
    ...
    generateTimeline() {
      ...
      this._geese.forEach((g) => {
        const data = g.getAttribute("goose-scripted");
        const id = data && (typeof data === "string" ? data : data.gid);
        ...
        const maxJumps = Math.min(
          this.data.jumpsPerGooseMax,
          1 + Math.floor(Math.random() * this.data.jumpsPerGooseMax)
        );
    ...
    exportTimeline() {
      ...
      try {
        const blob = new Blob([JSON.stringify(this.timeline, null, 2)], {
    ...
    tick() {
      return; // disabled during debugging
      if (!this.timeline) return;
      ...
      this.timeline.geese.forEach((entry) => {
        ...
        const nearPlayer =
          dx * dx + dz * dz <
          this.data.playerProximityOverride * this.data.playerProximityOverride;
        ...
        for (let i = 0; i < path.length - 1; i++) {
          if (loopT >= path[i].t && loopT <= path[i + 1].t) {
            aIdx = i;
            bIdx = i + 1;
            break;
          }
